...
export default class CategoryRepositoryPostgres extends BaseRepositoryPostgres implements CategoryRepositoryInterface {
    ...
    async getCategories(child: boolean, parentId: string): Promise<CategoryModel[]> {
        const where = {isDeleted: false} as any;
        if (child === false) {
            where.parentId =  IsNull();
        }
        if (child === true && parentId) {
            where.parentId = parentId;
        }
        const categories = await this.getRepository(CategoryEntity).find({where});
        return CategoryMapper.fromEntitiesToModels(categories);
    }
    ...
    async deleteCategory(id: string): Promise<void> {
        await this.getRepository(CategoryEntity).delete({id});
        return;
    }
